...
function createGameBoard() {
    ...
    return  {
        ...
        move( ship, direction) {
            ...
            switch(direction) {
                case "right":
                    newShip = createShip( ship.length, ship.posArray.map( x => [x[0] + 1, x[1] ] ), ship.orientation);
                    break;
                case "left":
                    newShip = createShip( ship.length, ship.posArray.map( x => [x[0] - 1, x[1] ] ), ship.orientation);
                    break;
                case "down":
                    newShip = createShip( ship.length, ship.posArray.map( x => [x[0] , x[1] + 1 ] ), ship.orientation);
                    break;
                case "up" :
                    newShip = createShip( ship.length, ship.posArray.map( x => [x[0] , x[1] - 1 ] ), ship.orientation);
                    break;
